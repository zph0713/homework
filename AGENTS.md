...
| 🎯 雅思专项训练（ielts_reading / ielts_listening / ielts_essay / ielts_speaking） | 仿真剑桥真题出题：阅读节选小题（原文+标准题组：TFNG / 单选 / 填空）、听力精听（**精听文稿 + 真题格式题目**：单选 / 表格填空等，题区说明行 + 真题题号 + 表格标准化，见 docs/QUESTION_TYPES.md「真题格式卷」）、作文长句中译英（大作文模版句+小作文图表例句）、口语随机话题；**四栏目时刻补齐（见下节）** | 阅读节选小题 / 听力精听：学生直接作答，choice / tfng 交卷自动批改即定论，fill / cloze 错漏由老师复核并附**答案讲解（定位句）**；作文句翻译批改**纠正语法**；口语**只出题不批改**（练完前端记「完成」=已做过；Part2 模拟真题卡，优先当季话题，Part3 老师自行出题） |
...
- **目标**：**词汇短语作业**（`vocabulary`）与雅思专项 4 个子栏目（`ielts_reading` 阅读节选小题 / `ielts_listening` 听力精听·文稿+题目 / `ielts_essay` 作文中译英 / `ielts_speaking` 口语话题）**每栏时刻保持 ≥1 张「未做」作业卡**，学生任何时候打开都有得练。
...
- **分栏目批改口径**：语法作业全部批改讲解；词汇短语作业不批改（学生自验证）；雅思阅读节选小题 / 听力精听直接作答（choice / tfng 自动批改即定论，fill / cloze 错漏复核），批改附**答案讲解（定位句）**；作文长句中译英批改纠正语法问题；口语卷「完成练习」只记录为已做过（status=`done`），不进待批改、不批改
