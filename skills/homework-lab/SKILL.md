...
- 学生说「出题吧 / 来点练习 / 写好了 / 交了 / 交卷了」或提到作业、试卷、错题、知识点掌握度、单词本默写
...
3. **按学生画像定制**：出题前必读 `profile get`（目标/话题/题型/备注）+ `vocab list --unfilled`（顺手补词）。学生在「我的」页设置的雅思话题、翻译/阅读/写作需求直接决定题目方向；出题策略随错题与弱点动态调整。
...
python3 agent/cli.py vocab list --unfilled   # 单词本缺中文/词性的词
```

## 交互模式（用户约定）

- **出题**：学生主动说「出题吧 / 来点默写」才出题，不擅自布置。方向按「三大终极目标」。
...
3. **讲解 + 记录**（批改后必做）：
   ...
   - **单词本查漏**（批改后必做）：`vocab list --unfilled` 有词 → 写 /tmp/vocab.json 补齐中文+词性 → `vocab update /tmp/vocab.json`
   - 若本次有错 → 立刻出验证卷（第 4 步）
...
- 题型：choice / fill / cloze / tfng / writing / translate；默写=fill+skill=vocabulary。**listening/speaking 未实现，不要出**（预留见 docs/ROADMAP.md）
- 学生在网页可：划词加入单词本（自动）、看单词本 5 列表格（词/词性/中文/备注/时间）、「我的」页改画像、错题本申请重练、删除作业卡（网页有确认）、设置页改库路径/端口
...
- 学生画像：`profile get`；单词本：`vocab list [--unfilled]`
